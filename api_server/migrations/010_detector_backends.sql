...
-- SQLite cannot widen a CHECK constraint in place. Foreign keys are disabled
-- only for these table copies; Database.migrate runs foreign_key_check before
-- restoring normal enforcement.
...
CREATE TABLE recognition_regions_new (
  ...
  source TEXT NOT NULL DEFAULT 'craft'
    CHECK(source IN ('craft','kraken','manual','adjusted')),
...
CREATE TABLE recognition_run_regions_new (
  ...
  source TEXT NOT NULL CHECK(source IN ('craft','kraken','manual','adjusted')),
...
INSERT INTO recognition_run_regions_new(
  ...
  source,flags_json,detector_version,detector_score,page_revision,created_at
)
SELECT
  id,recognition_run_id,page_id,source_region_id,polygon_json,reading_order,
  source,flags_json,detector_version,detector_score,page_revision,created_at
FROM recognition_run_regions;
...
CREATE TABLE worker_model_readiness_new (
  ...
  model_name TEXT NOT NULL CHECK(model_name IN ('craft','kraken','trocr')),
...
INSERT INTO worker_model_readiness_new(
  worker_id,model_name,status,model_version,evidence_json,error_code,warmed_at,updated_at
)
SELECT worker_id,model_name,status,model_version,evidence_json,error_code,warmed_at,updated_at
FROM worker_model_readiness;

DROP TABLE worker_model_readiness;
ALTER TABLE worker_model_readiness_new RENAME TO worker_model_readiness;

ALTER TABLE recognition_runs ADD COLUMN detector_name TEXT;
ALTER TABLE recognition_runs ADD COLUMN detector_version TEXT;
ALTER TABLE recognition_runs ADD COLUMN detector_config_json TEXT;

CREATE TABLE detector_outputs (
  id TEXT PRIMARY KEY,
