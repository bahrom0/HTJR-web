    ...
    <section className="home-page" aria-labelledby="home-title">
      ...
      >
        <div className="home-hero__copy">
          ...
          <p className="home-hero__lead">
            Оригинал остаётся неизменным, каждый этап сохраняется отдельно, а результат всегда можно
            проверить вручную.
          </p>
        ...
        <aside className="session-card" aria-label="Состояние подключения">
          <div className="session-card__visual" aria-hidden="true">
            <Icon name="shield" />
            <span />
            <Icon name="document" />
          </div>
          ...
          <Status tone={isOnline ? 'success' : 'warning'}>
            {isOnline
              ? 'Сервер отвечает через защищённую сессию'
              : 'Серверные действия временно недоступны'}
          </Status>
          <dl className="session-card__facts">
            <div>
              <dt>Доступ</dt>
              <dd>{access.state === 'authenticated' ? 'Разрешён' : 'Проверяется'}</dd>
            </div>
            <div>
              <dt>Сессия до</dt>
              <dd>{formatExpiry(access.expiresAt)}</dd>
            </div>
            <div>
              <dt>Хранение</dt>
              <dd>На вашем сервере</dd>
            </div>
          </dl>
        </aside>
      </motion.div>
      ...
      <section className="home-section" aria-labelledby="workflow-title">
        <div className="section-heading">
          <div>
            <p className="eyebrow">Путь документа</p>
            <h2 id="workflow-title">Четыре понятных этапа</h2>
          </div>
          <p>
            Никаких скрытых изменений: подготовка, распознавание и исправления сохраняются
            раздельно.
          </p>
        </div>
        <div className="workflow-grid">
          {workflow.map(([number, title, description, icon], index) => (
            <motion.article
              ...
              transition={{ ...motionTransition.enter, delay: canAnimate ? index * 0.04 : 0 }}
            >
      ...
      <section className="home-lower-grid">
        ...
        <Card className="privacy-card">
          ...
          <h2>Изображения не покидают ваш сервер</h2>
          <p>
            Код доступа одноразовый, токен недоступен странице, а каждый объект привязан к своей
            сессии.
          </p>
          <Link to="/settings">
            Настройки и приватность <Icon name="arrow" />
          </Link>
        </Card>
      </section>
    </section>
