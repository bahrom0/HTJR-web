    ...
    <section className="access-page" aria-labelledby="access-title">
      ...
      >
        <div className="access-story">
          ...
          <p>
            Браузер подключается к вашему локальному серверу. Изображения, тексты и исправления
            остаются под вашим контролем.
          </p>
          <div className="access-connection" aria-label="Схема подключения">
            <span>Браузер</span>
            <i />
            <Icon name="shield" />
            <i />
            <span>Ваш сервер</span>
          </div>
          <ul className="access-benefits">
            ...
            <li>
              <Icon name="document" />
              Раздельные версии текста
            </li>
          </ul>
        </div>
        <div className="access-form-card">
          <p className="eyebrow">Защищённое подключение</p>
          <h2>Введите временный код</h2>
          <p>Создайте код в терминале сервера. После входа вы вернётесь на открытый ранее экран.</p>
          <form className="access-form" onSubmit={submit}>
            <Field
              label="Код доступа"
              hint="Формат: XXXX-XXXX-XXXX"
              name="access-code"
              value={code}
              ...
              required
            />
            <Button type="submit" isLoading={submitting} aria-label="Подключиться к серверу">
              {submitting ? 'Подключаемся…' : 'Подключиться к серверу'}
              <Icon name="arrow" />
            </Button>
          </form>
          {error ? (
            <Button variant="quiet" onClick={() => void access.reconnect()}>
              Проверить соединение
            </Button>
          ) : null}
          <Status tone="info">
            Код и токен сессии не сохраняются в локальном хранилище браузера.
          </Status>
        </div>
      </motion.div>
    </section>
